...
with open(fullFilePath, encoding="utf8") as fil:
    ...
    for linje in fil:
        ...
        Akershus_kommuner_skatt.append(temp_kommune)

#b) 
print(f"\nb) Skriver ut sortert liste\n")
Akershus_kommuner_skatt.sort(reverse=True, key = lambda kommune: kommune[1])

print(f"{'Kommune':15}{'Skattepliktige':>20} {'Inntekter':>22}")
for kommune, antall, inntekt in Akershus_kommuner_skatt:
...
if len(soketerm) > 0:
    if soketerm[0]==soketerm[-1]=="*":
        ...
        for kommune, antall, skatt in Akershus_kommuner_skatt:
...
for i in range(len(Akershus_kommuner_skatt)):
    Akershus_kommuner_skatt[i].append(Akershus_kommuner_skatt[i][2]/Akershus_kommuner_skatt[i][1])
